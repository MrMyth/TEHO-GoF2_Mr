...
string descF26 = "+35 points for skills.";
...
string ConvertPointForTool(float valve)
{
int iFindPos;
string text = ConvertToText(valve);
iFindPos = findSubStr(text, ".", 0) - 1;
string FirstP  = strcut(text, 0, iFindPos);
iFindPos = findSubStr(text, ".", 0) + 1;
string SecP  = strcut(text, iFindPos, strlen(text) - 1);
if(SecP == "")
{
SecP = 0;
}
...
string ConvertToText (float valve)
{
return ""+valve;
}
